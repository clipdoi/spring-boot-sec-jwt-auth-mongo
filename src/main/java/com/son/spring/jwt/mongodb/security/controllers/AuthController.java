	...
	public ResponseEntity<?> logoutUser(HttpServletRequest request) {
		try {
			...
			if (jwtUtils.validateJwtToken(jwt)) {
				long ttlMillis = 3600000; // Thời gian sống của token (1 giờ)
				blacklistService.addTokenToBlacklist(jwt, ttlMillis);
				return ResponseEntity.ok("Logout successful and token revoked");
			} else {
